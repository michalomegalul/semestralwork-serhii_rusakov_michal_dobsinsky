"""
analysis_changepoint.py - Detekce bodů změny v měsíční časové řadě.

Metody: PELT (automatický počet bodů) + Binary segmentation (záložní).
Pracujeme s měsíčními agregáty — denní řada je příliš zašuměná.

Výstup: outputs/changepoint.txt, outputs/changepoint.png
"""
...
def run() -> None:
    ...
    def _to_dates(cps: list[int]) -> list[pd.Timestamp]:
        # ruptures vrací indexy včetně koncového — odřízneme
